...
async function loadAnimeForEditing(id, uploaderEmail) {
    ...
    const effectiveUploader = uploaderEmail ||
        SUPER_ADMIN_EMAIL;
    const isMine = (effectiveUploader === currentUserEmail);
    const isSuperAdmin = (currentUserEmail === SUPER_ADMIN_EMAIL);
...
async function subirAGithHub() {
    ...
    try {
        ...
        // ===============================================
        // MODIFICACIÓN: NO RECARGAR, ACTIVAR ALERTA
        // ===============================================
        log("✨ ¡EXITO!");
        showToast("¡Proceso Completado! Cierre sesión.", false);
        
        // Mensaje final en el log
        document.getElementById('statusLog').innerHTML += `<br><br><span style="color:#ff4757; font-weight:bold; font-size:1.1em;">🛑 SUBIDA FINALIZADA.</span><br>Por favor, haz clic en el botón de <b style="color:#ff4757">APAGADO</b> (arriba a la derecha) para recargar y ver los cambios.`;
        
        // Activar efecto visual en el botón de logout
        const logoutBtn = document.getElementById('logoutBtn');
        if (logoutBtn) {
            logoutBtn.classList.add('pulse-alert');
            // Hacer scroll hacia arriba para que vea el botón
